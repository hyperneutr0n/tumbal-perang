...
<body class="bg-gray-100">
  <div class="min-h-screen">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        <div class="flex items-center gap-8">
          ...
          <div class="flex gap-6">
            ...
            <a href="{{ route('attack.list') }}" class="text-blue-600 hover:text-blue-800 font-semibold">Attack</a>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      @if (session('attack_result'))
        ...
        <div
          class="mb-6 p-4 rounded {{ $result['status'] === 'win' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
          @if ($result['status'] === 'win')
            <strong>Attack Success!</strong> You stole {{ $result['stolen_gold'] }} gold.
          @else
            <strong>Attack Failed!</strong> All your troops died. Defender has {{ $result['defender_survivors'] }}
            troops left.
          @endif
        </div>
      @endif
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($targets as $target)
          <div class="bg-white rounded-lg shadow-md p-6 flex flex-col" id="target-card-{{ $target->id }}">
            ...
            <button onclick="attackTarget({{ $target->id }}, '{{ $target->username }}')"
        ...
        @empty
          <div class="col-span-full text-center py-12">
            ...
            <p class="text-gray-400 text-sm mt-2">Players must have at least one barrack and one gold mine to be
              attackable.</p>
          </div>
        @endforelse
      </div>
  ...
  <script>
    async function attackTarget(targetId, targetName) {
      ...
      try {
        const terrainResponse = await fetch('{{ url('/attack/terrain') }}', {
          method: 'GET',
          headers: {
        ...
        const randomTerrain = terrainData.terrain;
        ...
        if (data.status === 'win') {
          ...
          resultDiv.innerHTML =
            `<strong>Victory!</strong><br>Terrain: ${data.terrain}<br>Stole ${data.stolen_gold} gold!`;
